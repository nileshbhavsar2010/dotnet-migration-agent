...
description: >-
  Applies the update specification produced by net-migration-analyzer to
  project files (global.json, .csproj) in a single batched operation, with
  automatic per-file backup and rollback on failure. Use after
  net-migration-analyzer has produced an update spec and changes are ready
